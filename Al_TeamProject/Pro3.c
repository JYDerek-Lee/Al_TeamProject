	...
	for (round = 1; round <= 2; round++) {  
		for (i = 0; i<7; i++) {
			for (k = 0; k<7; k++) {
				...
				if (findRoom(roomData) == ROOM) {
					for (del = 0; del<49; del++) {
...
void aiPlayer() {
	...
	int recentX = (g_save[g_saveNum - 1] / 100) % 10; 
	...
	if (g_saveNum != 1) { 
		if (recentX - 1 != befroeRecentX || recentX + 1 != befroeRecentX || recentY - 1 != befroeRecentY || recentY + 1 != befroeRecentY) { 
